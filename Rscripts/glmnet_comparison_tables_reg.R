...
dsr <- dsr[!grepl("TRUE|0.75|mae", dsr)]
...
reg_results <- dplyr::group_by(reg_data, data, package, method, optimizer,
                               assess, tuned_on) %>%
  dplyr::summarise(rmse = median(acc_rmse, na.rm = TRUE),
                   mae = median(auc_mae, na.rm = TRUE),
                   time = median(time, na.rm = TRUE))

reg2 <- select(reg_results, -tuned_on, -mae)
...
write.csv(reg_tab, "../tables/glmnet_reg.csv", row.names = FALSE, quote = FALSE)

# make a table with only rmse
reg_rmse <- data.frame(Data = reg2$data, mod = mod, rmse = reg2$rmse)
reg_rmse <- pivot_wider(reg_rmse, names_from = mod, values_from = rmse)
reg_rmse

reg_time <- data.frame(Data = reg2$data, mod = mod, time = reg2$time)
reg_time <- pivot_wider(reg_time, names_from = mod, values_from = time)
reg_time

write.csv(reg_rmse, "../data/tables/glmnet_reg_rmse.csv", row.names = FALSE, quote = FALSE)
write.csv(reg_time, "../data/tables/glmnet_reg_time.csv", row.names = FALSE, quote = FALSE)
